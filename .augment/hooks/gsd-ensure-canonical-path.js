...
// hardcode the canonical path `@~/.augment/gsd-core/...` (references, workflows,
...
// `~/.augment/gsd-core/` is never created and every @-include resolves to
...
// `~/.augment/gsd-core/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.augment/gsd-core/<subdir>/...` includes point into.
...
function resolveConfigDir(homeDir, env) {
  ...
  return path.join(homeDir, '.augment');
}
...
/**
 * Ensure the canonical `~/.augment/gsd-core/` path exposes the bundled subdirs.
 *
