...
The keys live in `.claude/settings.json` (project) or `/Users/smintz/go/src/github.com/protoconf/protoconf/.kilo/settings.json` (global) — **not** in `.planning/config.json`:
...
See @/Users/smintz/go/src/github.com/protoconf/protoconf/.kilo/gsd-core/references/planner-guidance.md for Granularity Calibration table (Coarse/Standard/Fine plans-per-phase).
