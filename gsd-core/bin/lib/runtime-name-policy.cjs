 ...
 * (opencode/kilo/kimi) incorrectly emitting `.claude/.clinerules`. Pure: no I/O
 ...
 * ids fall back to 'Cline' (the always-safe default, fail-closed).
 *
 ...
 * 'Cline'. Sibling to `getDirName`; pure (no I/O).
 */
function getRuntimeLabel(runtime) {
    if (!runtime)
        return 'Cline';
    const label = RUNTIME_LABELS[runtime];
    return typeof label === 'string' && label.length > 0 ? label : 'Cline';
}
/**
