...
// Tool name mapping from Cline to OpenCode
...
/**
 * Convert a Cline tool name to OpenCode format
 ...
 * by Cline as the skill identity. Emits the hyphen form (gsd-<cmd>)
 * so Cline autocomplete shows the canonical invocation form, not the
 ...
 * Cline is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // `name:` Cline (and Qwen/Hermes) register under (#2808). `cmdNames`
 ...
 *
 * WHY this can't stay a static `@`-include (even a relative one): Cline
 * documents relative `@`-paths as resolving against the file *containing* the
 * import, which for a global skill is `~/.cline/skills/gsd-<stem>/` — not
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.cline/skills/ → ~/.gemini/config/skills/ (#3738),
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Remove Cline-specific bug workarounds before brand replacement
    converted = converted.replace(/\*\*Known Cline bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cline" brand references with "Cursor" — #2284(b): skips
...
// without this would have silently regressed live installs: `Cline`
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cline bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cline" brand references with "Windsurf" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Cline bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Cline" brand references with "Augment" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // unprefixed `./.clinerules` form the very next pattern handles differently
...
function convertClaudeToCodebuddyMarkdown(content) {
    ...
    // CodeBuddy uses the same tool names as Cline (Bash, Edit, Read, Write, etc.)
...
/**
 * Convert a Cline slash-command (.md) to a CodeBuddy slash-command (.md).
 *
...
function convertClaudeToCliineMarkdown(content) {
    ...
    // Cline uses the same tool names as Cline — no tool name conversion needed
...
function convertClaudeToCodexMarkdown(content) {
    ...
    // "check `.cline/skills/`" where there is no ~/, $HOME/, or ./ anchor.
...
GSD workflows use \`AskUserQuestion\` (Cline syntax). Translate to Codex \`request_user_input\`:
...
GSD workflows use \`Task(...)\` (Cline syntax). Translate to Codex collaboration tools:
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Negative lookahead avoids: Cline, Claude Opus/Sonnet/Haiku, Claude native, Claude-based
    c = c.replace(/\bClaude(?! Code| Opus| Sonnet| Haiku| native| based|-)\b(?!\.md)/g, 'the agent');
    // Replace .clinerules with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null, variant = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — OpenCode doesn't support Cline model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    convertedContent = replaceRelativePathReference(convertedContent, '.cline/skills/', '.kilo/skills/');
    ...
    for (const line of lines) {
        ...
        // Strip model: field — Kilo doesn't support Cline model aliases
...
// Copilot tool name mapping — Cline tools to GitHub Copilot tools
...
// Tool name mapping from Cline to Gemini CLI
...
/**
 * Convert a Cline tool name to Gemini CLI format
...
/**
 * Convert a Cline tool name to GitHub Copilot format.
...
/**
 * Convert Cline agent markdown to Cursor agent format.
...
/**
 * Convert Cline agent markdown to Windsurf agent format.
...
/**
 * Convert Cline agent markdown to Augment agent format.
 ...
 *
 * `color` IS preserved: Qwen's docs list `color` under "Cline
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (.clinerules /
 * Cline / .claude/ literal-substring values — descriptor-driven via
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    let converted = content;
    if (_b['.clinerules'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.clinerules']);
    // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
    if (_b['Cline'])
        converted = applyClaudeCodeBrandSwap(converted, _b['Cline']);
    if (_b['.claude/'])
...
/**
 * Convert a Cline agent .md for ZCode (#3384).
 *
 ...
 * not connected") whenever it is not connected, whereas Cline treats the
 ...
 *   .clinerules    -> brandingRewrites['.clinerules']
 *   Cline  -> brandingRewrites['Cline']  (word-boundary, \bClaude Code\b)
...
function applyAgentBrandingRewrites(content, runtime) {
    ...
    if (_b['.clinerules'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.clinerules']);
    if (_b['Cline'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Cline']);
    if (_b['.claude/'])
        converted = converted.replace(/\.claude\//g, _b['.claude/']);
    return converted;
}
...
/**
 * Convert Cline agent markdown to Codex agent format.
 ...
 * non-Claude runtime, because GSD's worktree isolation was Cline's
 ...
 * not expand inside double-quoted shell strings (#1284). But Cline's
 ...
 * automated test can verify *resolution* inside a live Cline session
 ...
 * `~` form Cline documents as expanding. A single pathPrefix string
 ...
 * Cline documents `@`-references as valid "anywhere in your
 * .clinerules" (e.g. `See @README for project overview`), so anchoring to
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (.clinerules/Cline/.claude/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['.clinerules']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['.clinerules']);
                // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
                content = applyClaudeCodeBrandSwap(content, _b['Cline']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
    ...
    // #3533 (10d): 'inherit' means the effort: key must NOT exist — Cline
