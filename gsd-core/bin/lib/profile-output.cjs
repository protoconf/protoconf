...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    // hardcoded .claude/.clinerules. Mirrors the #3163 fix applied to
    ...
    //   write to .clinerules even if a prior Claude setup left claude_md_path).
    let config = {};
    try {
    ...
    if (options.global) {
        if (isClaudeRuntime) {
            targetPath = node_path_1.default.join(node_os_1.default.homedir(), '.claude', '.clinerules');
        }
        else {
        ...
        // Read claude_md_path from config; #1098 default is .claude/.clinerules
        ...
        let configClaudeMdPath = '.claude/.clinerules';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.claude/.clinerules` (a valid auto-loaded memory location) rather than a
    // repo-root `.clinerules`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root .clinerules. An explicit `claude_md_path`
    ...
    try {
        ...
        // .clinerules; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to .clinerules
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted .clinerules/AGENTS.md, not a
