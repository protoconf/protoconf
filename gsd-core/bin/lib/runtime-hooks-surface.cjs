 ...
 * 'Cline' : runtime` ternary — cosmetic (log text) only, but resolves
 * to the same 'Qwen Code' / 'Cline' values for those two runtimes.
...
function applySettingsJsonHooks(settings, opts) {
    ...
    if (hooksSurface !== 'none' && hooksSurface !== 'kimi-hooks-toml') {
        ...
        // #3981: Cline treats a timed-out hook as NON-blocking — the tool
        ...
        // write (#768): on Cline >= 2.1.259 ANY Read() deny rule makes every
        ...
        //    (#788 + #770 + #2092) ────────────────────────────────────────────────
        // Cline (since #770) and Qwen Code (since #788) both support the
        ...
        // Cline fires FileChanged when a watched file changes on disk.  Wire
        ...
        // (Cline matches by filename, not full path).  The hook exits silently
        // when the changed file is not the gsd config.
        //
        // Scoped to Cline only: Qwen Code's FileChanged support is not yet
        ...
        if (extendedEvents.includes('FileChanged')) {
            ...
            if (!alreadyHasConfigReload && node_fs_1.default.existsSync(configReloadFile) && configReloadCommand) {
                ...
                console.log(`  ${green}✓${reset} Configured FileChanged config-reload hook (Cline)`);
            }
