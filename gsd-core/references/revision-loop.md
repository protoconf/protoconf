...
```markdown
- [ ] REVISION_CONFLICT {dimension}/{plan} — required_property: {property} | conflicts with: {locked decision D-nn / .clinerules rule / plan constraint} | alternatives: {the agent's alternatives}
```
