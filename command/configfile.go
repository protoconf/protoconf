...
// matchesBase is superseded by ConfigLayerer's provenance set (see below) and is retained only
// until 08-06 migrates the remaining four components (compiler, server, inserter, mutate) off
// the free LayerConfigFile function. Patching its composite arm with proto.Equal
// (08-REVIEW.md CR-01's suggested fix) would have fixed the message-typed-field gap alone,
// while leaving the env-coincidence gap open, because the baseline it compares against (base)
// still accumulates config-file values — value equality against one evolving baseline cannot
// tell "explicitly supplied" apart from "carried over from an earlier file".
//
...
// Unlike the superseded LayerConfigFile/matchesBase pair, it does not infer provenance by
// comparing a candidate value against an accumulating baseline — 08-REVIEW.md CR-01 shows that
// approach cannot distinguish a value that was explicitly supplied by an env var or flag from
// one merely carried over from an earlier config file. Instead it records, once, which
// top-level fields were supplied by an env var or by an already-parsed flag, and that record
// persists across every subsequent -config-file so no later file can erase it.
