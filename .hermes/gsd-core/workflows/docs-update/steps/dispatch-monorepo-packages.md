...
Spawn a `gsd-doc-writer` agent with `background=true`:
...
> **ORCHESTRATOR RULE — CODEX RUNTIME**: After calling all per-package delegate_task() calls above with `background=true`, do NOT generate any package READMEs independently while the subagents are active. Wait for all agents to complete before proceeding. This prevents duplicate work and wasted context.

Collect confirmations by reading each package agent's `outputFile` once it reports completion — each `background=true` Agent call returns an `async_launched` result carrying an `outputFile` path (with `canReadOutputFile: true`). Note failures in the final report.
