 ...
 * (opencode/kilo/kimi) incorrectly emitting `.hermes/HERMES.md`. Pure: no I/O
 ...
 * ids fall back to 'Hermes Agent' (the always-safe default, fail-closed).
 *
 ...
 * 'Hermes Agent'. Sibling to `getDirName`; pure (no I/O).
 */
function getRuntimeLabel(runtime) {
    if (!runtime)
        return 'Hermes Agent';
    const label = RUNTIME_LABELS[runtime];
    return typeof label === 'string' && label.length > 0 ? label : 'Hermes Agent';
}
/**
