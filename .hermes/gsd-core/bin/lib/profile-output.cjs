...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    // hardcoded .hermes/HERMES.md. Mirrors the #3163 fix applied to
    ...
    //   write to HERMES.md even if a prior Claude setup left claude_md_path).
    let config = {};
    try {
    ...
    if (options.global) {
        if (isClaudeRuntime) {
            targetPath = node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'HERMES.md');
        }
        else {
        ...
        // Read claude_md_path from config; #1098 default is .hermes/HERMES.md
        ...
        let configClaudeMdPath = '.hermes/HERMES.md';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.hermes/HERMES.md` (a valid auto-loaded memory location) rather than a
    // repo-root `HERMES.md`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root HERMES.md. An explicit `claude_md_path`
    ...
    try {
        ...
        // HERMES.md; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to HERMES.md
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted HERMES.md/AGENTS.md, not a
