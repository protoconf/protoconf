...
```markdown
- [ ] REVISION_CONFLICT {dimension}/{plan} — required_property: {property} | conflicts with: {locked decision D-nn / .trae/rules/rules.md rule / plan constraint} | alternatives: {the agent's alternatives}
```
