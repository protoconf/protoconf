 ...
 * claude runtime, mirroring the `model_policy` path (#1144). Trae's
...
function resolveModelInternal(cwd, agentType) {
    ...
    if (tier && tier !== 'inherit') {
        ...
        if (policyModel) {
            ...
            if (!onClaude)
                return policyModel;
            // Trae's Agent tool takes tier aliases (opus/sonnet/haiku/fable),
            ...
            // than returning an ID Trae cannot spawn.
