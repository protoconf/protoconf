...
// Tool name mapping from Trae to OpenCode
...
/**
 * Convert a Trae tool name to OpenCode format
 ...
 * by Trae as the skill identity. Emits the hyphen form (gsd-<cmd>)
 * so Trae autocomplete shows the canonical invocation form, not the
 ...
 * Trae is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // `name:` Trae (and Qwen/Hermes) register under (#2808). `cmdNames`
 ...
 *
 * WHY this can't stay a static `@`-include (even a relative one): Trae
 * documents relative `@`-paths as resolving against the file *containing* the
 * import, which for a global skill is `~/.trae/skills/gsd-<stem>/` — not
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.trae/skills/ → ~/.gemini/config/skills/ (#3738),
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Remove Trae-specific bug workarounds before brand replacement
    converted = converted.replace(/\*\*Known Trae bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Trae" brand references with "Cursor" — #2284(b): skips
...
// without this would have silently regressed live installs: `Trae`
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Trae bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Trae" brand references with "Windsurf" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Trae bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Trae" brand references with "Augment" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // unprefixed `./.trae/rules/rules.md` form the very next pattern handles differently
...
function convertClaudeToCodebuddyMarkdown(content) {
    ...
    // CodeBuddy uses the same tool names as Trae (Bash, Edit, Read, Write, etc.)
...
/**
 * Convert a Trae slash-command (.md) to a CodeBuddy slash-command (.md).
 *
...
function convertClaudeToCliineMarkdown(content) {
    ...
    // Cline uses the same tool names as Trae — no tool name conversion needed
...
function convertClaudeToCodexMarkdown(content) {
    ...
    // "check `.trae/skills/`" where there is no ~/, $HOME/, or ./ anchor.
...
GSD workflows use \`AskUserQuestion\` (Trae syntax). Translate to Codex \`request_user_input\`:
...
GSD workflows use \`Task(...)\` (Trae syntax). Translate to Codex collaboration tools:
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Negative lookahead avoids: Trae, Claude Opus/Sonnet/Haiku, Claude native, Claude-based
    c = c.replace(/\bClaude(?! Code| Opus| Sonnet| Haiku| native| based|-)\b(?!\.md)/g, 'the agent');
    // Replace .trae/rules/rules.md with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null, variant = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — OpenCode doesn't support Trae model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    convertedContent = replaceRelativePathReference(convertedContent, '.trae/skills/', '.kilo/skills/');
    ...
    for (const line of lines) {
        ...
        // Strip model: field — Kilo doesn't support Trae model aliases
...
// Copilot tool name mapping — Trae tools to GitHub Copilot tools
...
// Tool name mapping from Trae to Gemini CLI
...
/**
 * Convert a Trae tool name to Gemini CLI format
...
/**
 * Convert a Trae tool name to GitHub Copilot format.
...
/**
 * Convert Trae agent markdown to Cursor agent format.
...
/**
 * Convert Trae agent markdown to Windsurf agent format.
...
/**
 * Convert Trae agent markdown to Augment agent format.
 ...
 *
 * `color` IS preserved: Qwen's docs list `color` under "Trae
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (.trae/rules/rules.md /
 * Trae / .claude/ literal-substring values — descriptor-driven via
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    let converted = content;
    if (_b['.trae/rules/rules.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.trae/rules/rules.md']);
    // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
    if (_b['Trae'])
        converted = applyClaudeCodeBrandSwap(converted, _b['Trae']);
    if (_b['.claude/'])
...
/**
 * Convert a Trae agent .md for ZCode (#3384).
 *
 ...
 * not connected") whenever it is not connected, whereas Trae treats the
 ...
 *   .trae/rules/rules.md    -> brandingRewrites['.trae/rules/rules.md']
 *   Trae  -> brandingRewrites['Trae']  (word-boundary, \bClaude Code\b)
...
function applyAgentBrandingRewrites(content, runtime) {
    ...
    if (_b['.trae/rules/rules.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.trae/rules/rules.md']);
    if (_b['Trae'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Trae']);
    if (_b['.claude/'])
        converted = converted.replace(/\.claude\//g, _b['.claude/']);
    return converted;
}
...
/**
 * Convert Trae agent markdown to Codex agent format.
 ...
 * non-Claude runtime, because GSD's worktree isolation was Trae's
 ...
 * not expand inside double-quoted shell strings (#1284). But Trae's
 ...
 * automated test can verify *resolution* inside a live Trae session
 ...
 * `~` form Trae documents as expanding. A single pathPrefix string
 ...
 * Trae documents `@`-references as valid "anywhere in your
 * .trae/rules/rules.md" (e.g. `See @README for project overview`), so anchoring to
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (.trae/rules/rules.md/Trae/.claude/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['.trae/rules/rules.md']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['.trae/rules/rules.md']);
                // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
                content = applyClaudeCodeBrandSwap(content, _b['Trae']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
    ...
    // #3533 (10d): 'inherit' means the effort: key must NOT exist — Trae
