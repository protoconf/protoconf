 ...
 * (opencode/kilo/kimi) incorrectly emitting `.trae/rules/rules.md`. Pure: no I/O
 ...
 * ids fall back to 'Trae' (the always-safe default, fail-closed).
 *
 ...
 * 'Trae'. Sibling to `getDirName`; pure (no I/O).
 */
function getRuntimeLabel(runtime) {
    if (!runtime)
        return 'Trae';
    const label = RUNTIME_LABELS[runtime];
    return typeof label === 'string' && label.length > 0 ? label : 'Trae';
}
/**
