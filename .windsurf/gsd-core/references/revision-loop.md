...
```markdown
- [ ] REVISION_CONFLICT {dimension}/{plan} — required_property: {property} | conflicts with: {locked decision D-nn / .windsurf/rules rule / plan constraint} | alternatives: {the agent's alternatives}
```
