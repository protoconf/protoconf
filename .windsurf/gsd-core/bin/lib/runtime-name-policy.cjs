 ...
 * (opencode/kilo/kimi) incorrectly emitting `.claude/.devin/rules`. Pure: no I/O
 ...
 * ids fall back to 'Windsurf' (the always-safe default, fail-closed).
 *
 ...
 * 'Windsurf'. Sibling to `getDirName`; pure (no I/O).
 */
function getRuntimeLabel(runtime) {
    if (!runtime)
        return 'Windsurf';
    const label = RUNTIME_LABELS[runtime];
    return typeof label === 'string' && label.length > 0 ? label : 'Windsurf';
}
/**
