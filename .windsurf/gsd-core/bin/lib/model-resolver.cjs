 ...
 * claude runtime, mirroring the `model_policy` path (#1144). Windsurf's
...
function resolveModelInternal(cwd, agentType) {
    ...
    if (tier && tier !== 'inherit') {
        ...
        if (policyModel) {
            ...
            if (!onClaude)
                return policyModel;
            // Windsurf's Agent tool takes tier aliases (opus/sonnet/haiku/fable),
            ...
            // than returning an ID Windsurf cannot spawn.
