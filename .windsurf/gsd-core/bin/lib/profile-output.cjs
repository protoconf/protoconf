 ...
 *   - generate-claude-profile: Developer Profile section in .devin/rules
 *   - generate-claude-md: full .devin/rules with managed sections
 *
...
// .devin/rules fallback / placeholder text — runtime-aware so emitted slash
...
function buildClaudeMdFallbacks(runtime) {
    return {
        ...
        skills: 'No project skills found. Add skills to any of: `.devin/skills/`, `.agents/skills/`, `.cursor/skills/`, `.github/skills/`, or `.codex/skills/` with a `SKILL.md` index file.',
    };
}
...
// ─── .devin/rules Section Generators ─────────────────────────────────────────────
 ...
 * (name + description) for each. Returns a table summary for .devin/rules so
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    // hardcoded .claude/.devin/rules. Mirrors the #3163 fix applied to
    ...
    //   write to .devin/rules even if a prior Claude setup left claude_md_path).
    let config = {};
    try {
    ...
    if (options.global) {
        if (isClaudeRuntime) {
            targetPath = node_path_1.default.join(node_os_1.default.homedir(), '.claude', '.devin/rules');
        }
        else {
        ...
        // Read claude_md_path from config; #1098 default is .claude/.devin/rules
        ...
        let configClaudeMdPath = '.claude/.devin/rules';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.claude/.devin/rules` (a valid auto-loaded memory location) rather than a
    // repo-root `.devin/rules`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root .devin/rules. An explicit `claude_md_path`
    ...
    try {
        ...
        // .devin/rules; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to .devin/rules
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted .devin/rules/AGENTS.md, not a
