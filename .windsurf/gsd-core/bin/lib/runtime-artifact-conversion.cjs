...
// Tool name mapping from Windsurf to OpenCode
...
/**
 * Convert a Windsurf tool name to OpenCode format
 ...
 * by Windsurf as the skill identity. Emits the hyphen form (gsd-<cmd>)
 * so Windsurf autocomplete shows the canonical invocation form, not the
 ...
 * Windsurf is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // `name:` Windsurf (and Qwen/Hermes) register under (#2808). `cmdNames`
 ...
 *
 * WHY this can't stay a static `@`-include (even a relative one): Windsurf
 * documents relative `@`-paths as resolving against the file *containing* the
 * import, which for a global skill is `~/.devin/skills/gsd-<stem>/` — not
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.devin/skills/ → ~/.gemini/config/skills/ (#3738),
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Remove Windsurf-specific bug workarounds before brand replacement
    converted = converted.replace(/\*\*Known Windsurf bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Windsurf" brand references with "Cursor" — #2284(b): skips
...
// without this would have silently regressed live installs: `Windsurf`
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Windsurf bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Windsurf" brand references with "Windsurf" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Windsurf bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Windsurf" brand references with "Augment" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // unprefixed `./.devin/rules` form the very next pattern handles differently
...
function convertClaudeToCodebuddyMarkdown(content) {
    ...
    // CodeBuddy uses the same tool names as Windsurf (Bash, Edit, Read, Write, etc.)
...
/**
 * Convert a Windsurf slash-command (.md) to a CodeBuddy slash-command (.md).
 *
...
function convertClaudeToCliineMarkdown(content) {
    ...
    // Cline uses the same tool names as Windsurf — no tool name conversion needed
 ...
 * (.claude/→.cline/, "Windsurf"→"Cline", etc.).
...
function convertClaudeToCodexMarkdown(content) {
    ...
    // "check `.devin/skills/`" where there is no ~/, $HOME/, or ./ anchor.
...
GSD workflows use \`AskUserQuestion\` (Windsurf syntax). Translate to Codex \`request_user_input\`:
...
GSD workflows use \`Task(...)\` (Windsurf syntax). Translate to Codex collaboration tools:
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Negative lookahead avoids: Windsurf, Claude Opus/Sonnet/Haiku, Claude native, Claude-based
    c = c.replace(/\bClaude(?! Code| Opus| Sonnet| Haiku| native| based|-)\b(?!\.md)/g, 'the agent');
    // Replace .devin/rules with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null, variant = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — OpenCode doesn't support Windsurf model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    convertedContent = replaceRelativePathReference(convertedContent, '.devin/skills/', '.kilo/skills/');
    ...
    for (const line of lines) {
        ...
        // Strip model: field — Kilo doesn't support Windsurf model aliases
...
// Copilot tool name mapping — Windsurf tools to GitHub Copilot tools
...
// Tool name mapping from Windsurf to Gemini CLI
...
/**
 * Convert a Windsurf tool name to Gemini CLI format
...
/**
 * Convert a Windsurf tool name to GitHub Copilot format.
...
/**
 * Convert Windsurf agent markdown to Cursor agent format.
...
/**
 * Convert Windsurf agent markdown to Windsurf agent format.
...
/**
 * Convert Windsurf agent markdown to Augment agent format.
 ...
 *
 * `color` IS preserved: Qwen's docs list `color` under "Windsurf
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (.devin/rules /
 * Windsurf / .claude/ literal-substring values — descriptor-driven via
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    let converted = content;
    if (_b['.devin/rules'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.devin/rules']);
    // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
    if (_b['Windsurf'])
        converted = applyClaudeCodeBrandSwap(converted, _b['Windsurf']);
    if (_b['.claude/'])
...
/**
 * Convert a Windsurf agent .md for ZCode (#3384).
 *
 ...
 * not connected") whenever it is not connected, whereas Windsurf treats the
 ...
 *   .devin/rules    -> brandingRewrites['.devin/rules']
 *   Windsurf  -> brandingRewrites['Windsurf']  (word-boundary, \bClaude Code\b)
...
function applyAgentBrandingRewrites(content, runtime) {
    ...
    if (_b['.devin/rules'])
        converted = converted.replace(/CLAUDE\.md/g, _b['.devin/rules']);
    if (_b['Windsurf'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Windsurf']);
    if (_b['.claude/'])
        converted = converted.replace(/\.claude\//g, _b['.claude/']);
    return converted;
}
...
/**
 * Convert Windsurf agent markdown to Codex agent format.
 ...
 * non-Claude runtime, because GSD's worktree isolation was Windsurf's
 ...
 * not expand inside double-quoted shell strings (#1284). But Windsurf's
 ...
 * automated test can verify *resolution* inside a live Windsurf session
 ...
 * `~` form Windsurf documents as expanding. A single pathPrefix string
 ...
 * Windsurf documents `@`-references as valid "anywhere in your
 * .devin/rules" (e.g. `See @README for project overview`), so anchoring to
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (.devin/rules/Windsurf/.claude/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['.devin/rules']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['.devin/rules']);
                // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
                content = applyClaudeCodeBrandSwap(content, _b['Windsurf']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
    ...
    // #3533 (10d): 'inherit' means the effort: key must NOT exist — Windsurf
