 ...
 * claude runtime, mirroring the `model_policy` path (#1144). Qwen Code's
...
function resolveModelInternal(cwd, agentType) {
    ...
    if (tier && tier !== 'inherit') {
        ...
        if (policyModel) {
            ...
            if (!onClaude)
                return policyModel;
            // Qwen Code's Agent tool takes tier aliases (opus/sonnet/haiku/fable),
            ...
            // than returning an ID Qwen Code cannot spawn.
