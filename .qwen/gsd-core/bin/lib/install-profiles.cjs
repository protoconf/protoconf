...
// stripped at emit BEFORE any path rewrite or converter runs, so a `.qwen/` ->
 ...
 * staged directory (`rewriteStagedSkillBodies`, surface.cts): a `~/.qwen/`
 ...
 *   1. applyAgentPathRewrites   (4 base ~/.qwen/ regexes; skipped for copilot/antigravity)
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (agentCtx) {
                ...
                // Step 1: path rewrites (4 base ~/.qwen/ regexes; skipped for copilot/antigravity)
