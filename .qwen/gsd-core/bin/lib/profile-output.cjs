...
function cmdGenerateClaudeProfile(cwd, options, raw) {
    ...
    // hardcoded .qwen/QWEN.md. Mirrors the #3163 fix applied to
    ...
    //   write to QWEN.md even if a prior Claude setup left claude_md_path).
    let config = {};
    try {
    ...
    if (options.global) {
        if (isClaudeRuntime) {
            targetPath = node_path_1.default.join(node_os_1.default.homedir(), '.claude', 'QWEN.md');
        }
        else {
        ...
        // Read claude_md_path from config; #1098 default is .qwen/QWEN.md
        ...
        let configClaudeMdPath = '.qwen/QWEN.md';
...
function cmdGenerateClaudeMd(cwd, options, raw) {
    ...
    // `.qwen/QWEN.md` (a valid auto-loaded memory location) rather than a
    // repo-root `QWEN.md`, so generated GSD content does not land next to — or
    // pollute — a hand-crafted repo-root QWEN.md. An explicit `claude_md_path`
    ...
    try {
        ...
        // QWEN.md; copilot now resolves to .github/copilot-instructions.md, and
        ...
        // rationale: a Codex/AGENTS-native project must never write to QWEN.md
    ...
    if (existingContent === null) {
        ...
        // section markers — it is a hand-crafted QWEN.md/AGENTS.md, not a
