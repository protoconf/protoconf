 ...
 * (opencode/kilo/kimi) incorrectly emitting `.qwen/QWEN.md`. Pure: no I/O
 ...
 * ids fall back to 'Qwen Code' (the always-safe default, fail-closed).
 *
 ...
 * 'Qwen Code'. Sibling to `getDirName`; pure (no I/O).
 */
function getRuntimeLabel(runtime) {
    if (!runtime)
        return 'Qwen Code';
    const label = RUNTIME_LABELS[runtime];
    return typeof label === 'string' && label.length > 0 ? label : 'Qwen Code';
}
/**
