...
// Tool name mapping from Qwen Code to OpenCode
...
/**
 * Convert a Qwen Code tool name to OpenCode format
...
function convertClaudeToCopilotContent(content, isGlobal = false) {
    ...
    // Handle both `~/.qwen/foo` (trailing slash) and bare `~/.claude` forms in
 ...
 * by Qwen Code as the skill identity. Emits the hyphen form (gsd-<cmd>)
 * so Qwen Code autocomplete shows the canonical invocation form, not the
 ...
 * Qwen Code is the native format, so minimal conversion needed —
...
function convertClaudeCommandToClaudeSkill(content, skillName, runtime = null, cmdNames = null) {
    ...
    // `name:` Qwen Code (and Qwen/Hermes) register under (#2808). `cmdNames`
...
// `@~/.qwen/gsd-core/workflows/<stem>.md` include: line-start `@`, exact
 ...
 *
 * WHY this can't stay a static `@`-include (even a relative one): Qwen Code
 * documents relative `@`-paths as resolving against the file *containing* the
 * import, which for a global skill is `~/.qwen/skills/gsd-<stem>/` — not
 * the project's working directory. `@./.qwen/...` would therefore always
...
function resolveSpecRootReference(body) {
    if (typeof body !== 'string' || body.length === 0)
        return body;
    if (!body.includes('@~/.qwen/gsd-core/workflows/'))
        return body;
    ...
    return body.replace(WORKFLOW_SPEC_ROOT_INCLUDE_RE, (match, stem, cr, offset) => {
        ...
        return (`To load this command's workflow spec: check for ` +
            `\`.qwen/gsd-core/workflows/${stem}.md\` relative to the current working ` +
            `directory first (project-local); if it is not there, fall back to ` +
            `\`~/.qwen/gsd-core/workflows/${stem}.md\` (the global install). If ` +
 ...
 * Path mappings depend on install mode:
 *   Global: ~/.qwen/skills/ → ~/.gemini/config/skills/ (#3738),
 *          ~/.qwen/ → ~/.gemini/antigravity/, ./.qwen/ → ./.agents/
 *   Local:  ~/.qwen/ → .agents/, ./.qwen/ → ./.agents/
...
function convertClaudeToCursorMarkdown(content) {
    ...
    // Remove Qwen Code-specific bug workarounds before brand replacement
    converted = converted.replace(/\*\*Known Qwen Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Qwen Code" brand references with "Cursor" — #2284(b): skips
...
// without this would have silently regressed live installs: `Qwen Code`
...
function convertClaudeToWindsurfMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Qwen Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Qwen Code" brand references with "Windsurf" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
// `@~/.qwen/gsd-core/commands/gsd/<stem>.md` reference at a file that does
...
function convertClaudeCommandToWindsurfWorkflow(content, commandName) {
    ...
    return `# ${commandName}\n\n${effectiveDescription}\n\nRead and execute the GSD command at @~/.qwen/gsd-core/commands/gsd/${stem}.md end-to-end. Treat the user's message after /${commandName} as the command arguments.`;
}
// --- Augment converters ---
...
function convertClaudeToAugmentMarkdown(content) {
    ...
    converted = converted.replace(/\*\*Known Qwen Code bug \(classifyHandoffIfNeeded\):\*\*[^\n]*\n/g, '');
    converted = converted.replace(/- \*\*classifyHandoffIfNeeded false failure:\*\*[^\n]*\n/g, '');
    // Replace "Qwen Code" brand references with "Augment" — #2284(b): skips
    // <runtime_compatibility> comparison-table content (protected region).
...
function convertClaudeToTraeMarkdown(content) {
    ...
    // converter — for `~/.qwen/`, `$HOME/.qwen/`, AND `./.qwen/` alike —
    ...
    // left the doubled-prefix defect live for the `$HOME/.qwen/` and
    // `~/.qwen/` forms specifically (found the same way, one regression-test
    // run later). Scoped to a `.trae/` tail so it cannot also swallow the
    // unprefixed `./QWEN.md` form the very next pattern handles differently
...
function convertClaudeToCodebuddyMarkdown(content) {
    ...
    // CodeBuddy uses the same tool names as Qwen Code (Bash, Edit, Read, Write, etc.)
...
/**
 * Convert a Qwen Code slash-command (.md) to a CodeBuddy slash-command (.md).
 *
...
function convertClaudeToCliineMarkdown(content) {
    ...
    // Cline uses the same tool names as Qwen Code — no tool name conversion needed
 ...
 * (.qwen/→.cline/, "Qwen Code"→"Cline", etc.).
...
function convertClaudeCommandToClineSkill(content, skillName, _runtime = null, cmdNames = null) {
    ...
    // apply Cline-specific markdown rewrites (.qwen/→.cline/, etc.).
    const names = cmdNames || readGsdCommandNames();
    const normalizedBody = transformContentToHyphen(body, names);
...
GSD workflows use \`AskUserQuestion\` (Qwen Code syntax). Translate to Codex \`request_user_input\`:
...
GSD workflows use \`Task(...)\` (Qwen Code syntax). Translate to Codex collaboration tools:
...
function neutralizeAgentReferences(content, instructionFile) {
    ...
    // Negative lookahead avoids: Qwen Code, Claude Opus/Sonnet/Haiku, Claude native, Claude-based
    c = c.replace(/\bClaude(?! Code| Opus| Sonnet| Haiku| native| based|-)\b(?!\.md)/g, 'the agent');
    // Replace QWEN.md with runtime-appropriate instruction file
    if (instructionFile) {
...
function convertClaudeToOpencodeFrontmatter(content, { isAgent = false, modelOverride = null, variant = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — OpenCode doesn't support Qwen Code model aliases
...
function convertClaudeToKiloFrontmatter(content, { isAgent = false, modelOverride = null } = {}) {
    ...
    for (const line of lines) {
        ...
        // Strip model: field — Kilo doesn't support Qwen Code model aliases
...
// Copilot tool name mapping — Qwen Code tools to GitHub Copilot tools
...
// Tool name mapping from Qwen Code to Gemini CLI
...
/**
 * Convert a Qwen Code tool name to Gemini CLI format
...
/**
 * Convert a Qwen Code tool name to GitHub Copilot format.
...
/**
 * Convert Qwen Code agent markdown to Cursor agent format.
...
/**
 * Convert Qwen Code agent markdown to Windsurf agent format.
...
/**
 * Convert Qwen Code agent markdown to Augment agent format.
 ...
 *
 * `color` IS preserved: Qwen's docs list `color` under "Qwen Code
 ...
 *
 * Body: preserved verbatim after the qwen branding rewrite (QWEN.md /
 * Qwen Code / .qwen/ literal-substring values — descriptor-driven via
 * runtime.hostBehaviors.brandingRewrites, mirrors the qwen case in
 * _applyRuntimeRewrites). The anchored `~/.qwen/` / `$HOME/.qwen/` forms
...
function convertClaudeAgentToQwenAgent(content) {
    ...
    let converted = content;
    if (_b['QWEN.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['QWEN.md']);
    // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
    if (_b['Qwen Code'])
        converted = applyClaudeCodeBrandSwap(converted, _b['Qwen Code']);
    if (_b['.qwen/'])
        converted = converted.replace(/\.claude\//g, _b['.qwen/']);
    const { frontmatter, body } = extractFrontmatterAndBody(converted);
    if (!frontmatter)
        return converted;
    const name = extractFrontmatterField(frontmatter, 'name') || 'unknown';
...
/**
 * Convert a Qwen Code agent .md for ZCode (#3384).
 *
 ...
 * not connected") whenever it is not connected, whereas Qwen Code treats the
 ...
 *   QWEN.md    -> brandingRewrites['QWEN.md']
 *   Qwen Code  -> brandingRewrites['Qwen Code']  (word-boundary, \bClaude Code\b)
 *   .qwen/     -> brandingRewrites['.qwen/']
 *
...
function applyAgentBrandingRewrites(content, runtime) {
    ...
    if (_b['QWEN.md'])
        converted = converted.replace(/CLAUDE\.md/g, _b['QWEN.md']);
    if (_b['Qwen Code'])
        converted = converted.replace(/\bClaude Code\b/g, _b['Qwen Code']);
    if (_b['.qwen/'])
        converted = converted.replace(/\.claude\//g, _b['.qwen/']);
    return converted;
}
...
/**
 * Convert Qwen Code agent markdown to Codex agent format.
 ...
 * non-Claude runtime, because GSD's worktree isolation was Qwen Code's
 ...
 * context-sensitive correction layered on top of the blanket `~/.qwen/` /
 * `$HOME/.qwen/` -> pathPrefix substitution every Claude emit path
 ...
 * `node "$HOME/.qwen/gsd-core/bin/gsd-tools.cjs"`) need it, since `~` does
 * not expand inside double-quoted shell strings (#1284). But Qwen Code's
 ...
 * automated test can verify *resolution* inside a live Qwen Code session
 ...
 * `~` form Qwen Code documents as expanding. A single pathPrefix string
 ...
 * every `@~/.qwen/gsd-core/…` include in a global install's workflows/
 ...
 * literal `.qwen/` segment, so it silently no-opped for any global install
 ...
 * Qwen Code documents `@`-references as valid "anywhere in your
 * QWEN.md" (e.g. `See @README for project overview`), so anchoring to
 ...
 * like `echo "@$HOME/.qwen/x"`, where rewriting `$HOME` to `~` inside
...
function _applyRuntimeRewrites(content, runtime, pathPrefix, isGlobal = false, attribution = undefined) {
    ...
    switch (runtime) {
        ...
        // brand VALUES (QWEN.md/Qwen Code/.qwen/ replacements) now read from
        ...
        case 'qwen': {
            ...
            // rewrites below) instead of throwing on `_b['QWEN.md']`.
            const _b = _hostBehaviors(runtime).brandingRewrites;
            if (_b) {
                content = content.replace(/CLAUDE\.md/g, _b['QWEN.md']);
                // #2284(b): skips <runtime_compatibility> comparison-table content (protected region).
                content = applyClaudeCodeBrandSwap(content, _b['Qwen Code']);
            }
            content = content.replace(/~\/\.claude\//g, pathPrefix);
            content = content.replace(/\$HOME\/\.claude\//g, pathPrefix);
            ...
            if (_b) {
                content = content.replace(/\.claude\//g, _b['.qwen/']);
            }
            content = content.replace(/\.\/\.claude\//g, `./${dirName}/`);
        ...
        case 'zcode':
            ...
            // `${_GSD_RUNTIME_ROOT}/.qwen/…` matches none of these regexes and
 ...
 * unconditionally rewrites any bare (non-`@`-prefixed) `~/.qwen/` substring
 * in the body to the computed pathPrefix (`$HOME/.qwen/` for a global
 * install) and restores ONLY the `@`-prefixed form back to `~`
 * (`@$HOME/.qwen/` → `@~/.qwen/`). `resolveSpecRootReference`'s
 * replacement text is deliberately imperative prose containing a literal,
 * non-`@`-prefixed `~/.qwen/gsd-core/workflows/<stem>.md` — running it
 ...
 * `@~/.qwen/gsd-core/workflows/<stem>.md` include line (which survives the
...
/**
 * Apply the 4 base `~/.qwen/` path-prefix rewrites to a single agent content
 * string. Mirrors the inline agent loop in bin/install.js lines 9330-9340:
 *   ~/\.qwen/ → pathPrefix
 *   $HOME/\.qwen/ → pathPrefix
...
function applyAgentPathRewrites(content, runtime, pathPrefix) {
    ...
    // `@~/.qwen/...` include in a global Claude install shipped as `@$HOME/...`
    ...
    // fixes `@$HOME/.qwen/x` and leaves a bare `@$HOME/.claude` broken. The
...
function applyAgentFrontmatterExtensions(content, { runtime, agentName, targetDir }) {
    ...
    // #3533 (10d): 'inherit' means the effort: key must NOT exist — Qwen Code
