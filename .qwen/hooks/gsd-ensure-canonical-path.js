...
// hardcode the canonical path `@~/.qwen/gsd-core/...` (references, workflows,
...
// holding the bundled tree, so the includes resolve. In a Qwen Code
...
// `~/.qwen/gsd-core/` is never created and every @-include resolves to
...
// `~/.qwen/gsd-core/` exists and its immutable subdirs (bin, contexts,
...
// are the directories `@~/.qwen/gsd-core/<subdir>/...` includes point into.
 ...
 *
 * Qwen Code unpacks the package so the bundled tree sits at
...
/**
 * Ensure the canonical `~/.qwen/gsd-core/` path exposes the bundled subdirs.
 *
