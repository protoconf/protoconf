...
process.stdin.on('end', () => {
  ...
  try {
    ...
    // created linked worktree (plain non-GSD work, e.g. Qwen Code plan-mode) is
    ...
    // `path` authoritative: normalization returns early for native Qwen Code
    ...
    // or resolves them against the worktree CWD. Qwen Code's Edit/Write require
    ...
    if (fileTopResult.status !== 0 || !fileTopResult.stdout) {
      ...
      //  - Truly outside all git repositories (e.g. ~/.qwen/plans/) → not the
