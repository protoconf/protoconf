...
* **cli:** repeated `-config-file` flags now resolve so that the LATER file wins for message-typed fields (`agent`'s `tls-config` and `store-tls`) and for repeated-string fields (`insert`'s `store-address`), where previously the FIRST file's value silently persisted; and an environment variable now wins over a config file even when an earlier `-config-file` happened to set the same value, on all five components (`agent`, `serve`, `compile`, `insert`, `mutate`). This closes PCLI-09 for repeated `-config-file` occurrences. Security-relevant consequence: an operator running `protoconf agent -config-file base.json -config-file override.json` with different TLS material previously got `base.json`'s certificate, key and CA; it now gets `override.json`'s.
