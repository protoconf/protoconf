...
func testPutTTL(t *testing.T, kv store.Store, otherConn store.Store) {
	...
	err := otherConn.Put(firstKey, firstValue, &store.WriteOptions{TTL: 20 * time.Second})
	assert.NoError(t, err)

	// Put a second key with the Ephemeral flag
	err = otherConn.Put(secondKey, secondValue, &store.WriteOptions{TTL: 20 * time.Second})
	assert.NoError(t, err)
...
func testDeleteTree(t *testing.T, kv store.Store) {
	...
	pair, err := kv.Get(firstKey)
	if err != nil {
		t.Fatalf("could not get key pair: %v", err)
	}
	assert.NoError(t, err)
	if assert.NotNil(t, pair) {
		assert.NotNil(t, pair.Value)
